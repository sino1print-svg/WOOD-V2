...
import type { ValidatedPackagePlanIndex } from './package-plan-validation';
...
export interface ManifestBuildOptions {
  readonly validated: ValidatedPackagePlanIndex;
...
export function buildExportManifest(options: ManifestBuildOptions): ExportManifest {
  const { validated, versions, createdAt, exportId, contentEntries } = options;
