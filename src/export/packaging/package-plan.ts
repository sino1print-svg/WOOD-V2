...
import type { ExportFormatResult } from '../format-result';
...
function outputSlugMap(validated: ValidatedPackagePlanIndex): Map<string, string> {
  const map = new Map<string, string>();
  for (const sessionId of validated.plan.scope.sessionIds) {
...
export function hasScopePolicyViolation(plan: ExportPlan): boolean {
  return hasScopePolicyViolationWithIndex(plan);
}
...
function shouldEmitPairFile(
  ...
  if (!PAIR_FILE_ELIGIBLE_SCOPES.has(validated.plan.scope.scopeDetail)) return false;
...
export function buildPackageContentEntries(
  ...
  const readme: ExportFormatResult = formatMarkdown({
    planResult: { ok: true, value: trustedPlan },
    limits,
  });
  ...
  if (trustedPlan.selection.project !== null) {
    const built = buildProjectJson(trustedPlan.selection.project, contentLimits);
  ...
  for (const version of trustedPlan.selection.versions) {
  ...
  for (const artwork of trustedPlan.selection.artworks) {
  ...
  const slugs = outputSlugMap(validated);
  let sessionIndex = 0;
  ...
  for (const sessionId of trustedPlan.scope.sessionIds) {
    sessionIndex += 1;
    ...
    const sessionNumbering = (validated.numberingBySession.get(sessionId) ?? []).filter((item) =>
      shouldEmitPairFile(validated, sessionId, item.sceneId),
    );
    ...
    const validationResults = [
      ...(validated.validationResultsBySessionAndId.get(sessionId)?.values() ?? []),
    ];
  ...
  const rawProjectSlug = slugify(
    trustedPlan.selection.project?.name ?? trustedPlan.scope.projectId,
    trustedPlan.scope.projectId,
  );
