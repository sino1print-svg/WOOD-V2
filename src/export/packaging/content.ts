 ...
 */
import type { SessionId, ValidationFailure } from '../../shared/domain-model';
import type {
...
export function buildPromptMetadataJson(
  sessionId: SessionId,
  outputsAForSession: readonly ExportSelectedOutputA[],
  outputsBForSession: readonly ExportSelectedOutputB[],
  groupPlansForSession: readonly ExportSelectedGroupPlan[],
  coverForSession: ExportSelectedCover | undefined,
  limits: ContentLimits,
): BuiltTextDocument | null {
  const outputsA = outputsAForSession.map((item) => ({
    id: item.id,
    label: item.label,
    promptMeta: promptMetadataToJson(item.promptMeta),
  }));
  const outputsB = outputsBForSession.map((item) => ({
    id: item.id,
    label: item.label,
    promptMeta: promptMetadataToJson(item.promptMeta),
  }));
  const groupPlans = groupPlansForSession.map((item) => ({
  ...
  return serializeJson(
    {
      ...
      groupPlans,
      cover:
        coverForSession === undefined
          ? null
          : {
              id: coverForSession.id,
              promptMeta: promptMetadataToJson(coverForSession.promptMeta),
            },
    },
    limits,
  );
...
export function buildValidationJson(
  sessionId: SessionId,
  validationResultsForSession: readonly ExportSelectedValidationResult[],
  warnings: readonly ValidationFailure[],
  omissions: readonly ExportPlanOmission[],
  partial: boolean,
  limits: ContentLimits,
): BuiltTextDocument | null {
  const validationResults = validationResultsForSession.map((result) => ({
    id: result.id,
  ...
  const issues = warnings.map((issue) => {
  ...
  const omissionRows = omissions.map((omission) => {
