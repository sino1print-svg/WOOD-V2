...
function selectValidationResult(
  validation: ValidationResult,
  containingSessionId: SessionId,
): ExportSelectedValidationResult {
  return {
    id: validation.id,
    // The containing session is the authoritative identity anchor. Validation
    // result ids are session-local, and a copied source record must not retain
    // a stale foreign session id in the detached export projection.
      ...
      for (const validationId of Object.keys(session.validationResults).sort(compareUtf8)) {
        selectedValidations.push(
          selectValidationResult(
            session.validationResults[validationId as ValidationResultId]!,
            session.id,
          ),
        );
      }
