...
// Only meaningful when driven by `scripts/verify-golden-zips-external.mjs`
// with `GOLDEN_ZIP_DUMP_DIR` set - a no-op success in every other run (`npm
// run test`, the full repository suite, CI) so this file never affects the
// ordinary test count or requires an external `unzip` to be present.
it.skipIf(!process.env.GOLDEN_ZIP_DUMP_DIR)(
  'dumps every golden ZIP fixture, its ledger, and a reordered-hostile archive with its internal verification reason',
  () => {
